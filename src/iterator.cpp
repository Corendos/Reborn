...
Iterator make_iterator(ConstStringU8* str) { return make_iterator(str->data, str->size); }

Iterator make_iterator(StringU8* str) { return make_iterator(str->data, str->size); }

u64 get_position(Iterator* it) { return it->current - it->data; }

bool is_valid(Iterator* it) { return (it->current - it->data) < it->size; }
...
char peek(Iterator* it, u64 offset) { return peek_or(it, offset, 0); }
